...
**Approved by user:** yes — 2026-09-22, 2.1b review ("approved, continue").
...
**Context:** 2.3a flagged that the Selected Works project titles are clickable divs, unreachable by keyboard. On 2026-09-23 the owner asked to "make them keyboard-accessible". Implementing it surfaced a site-wide gap. globals.css draws focus outlines only under `body.keyboard-nav` and removes them otherwise (`body:not(.keyboard-nav) *:focus { outline: none }`), but nothing ever set that class. So no element on the site showed a focus indicator.
**Decision:**
1. Each project title (desktop list and phone list) holds a `<button type="button" class="titleButton">`. The item's onClick stays on its wrapper, so the mouse/touch target is unchanged, and keyboard Enter/Space clicks bubble to it. `.titleButton` is a block that inherits font, colour, stroke and alignment with no background, border or padding. Computed title type and rects were measured identical to 2.3a. Consequence (accepted by the request): the custom cursor now shows its hover ring over the titles. The active desktop title carries `aria-current="true"`.
...
## D-25 — Stable GPU layers: fewer redraws per Process stage change, off-screen sections release layers

**Context:** On 2026-09-23 the owner confirmed the pin is fixed (D-24) but said Process is still "really low performance on mobile". A layer census at a phone stage change found 94 composited layers page-wide. 28 of them were Manifesto word layers kept alive by CSS `will-change` while far off screen; the rest were Contact, Hero, Footer and Process's own. Reading the code also showed each stage change re-rasterised the full-screen Process panel twice (the giant stroked titles, with text shadows): once when the description words were lifted onto their own layers, and again when they were dropped back. Every flying icon was also rasterised twice (take-off, and again when its temporary layer was released).
**Decision:**
1. `.titlesList` and `.absoluteDescription` get permanent layers (`will-change: transform`), so title transitions and the word animation never dirty the full-screen panel.
2. Each icon becomes a layer at its final scale (invisible, one frame before its flight: `LAYER_LEAD`) and keeps it while its stage is shown. It is rasterised once and crisp at rest, and the flight only moves the bitmap. Icons of hidden stages drop their layers.
