...
4. Above-the-fold/LCP-adjacent images get `priority`. At audit there is none (hero is text+canvas); re-check after every layout change — if a section moves above the fold, its image inherits `priority`.
...
- **Budget (recorded in STATE.md at every phase gate):** homepage First Load JS ≤ **140 KB gzipped** after Phase 2.8 (stretch: 120 KB); no route may regress >5% between phase gates without a DECISIONS entry. Read numbers from the `next build` route table; paste the table into STATE.md.
- **Lighthouse procedure (local proxy):** `npm run build && npm run start`, then against `http://localhost:3000`: `npx lighthouse http://localhost:3000 --preset=perf --form-factor=mobile --screenEmulation.mobile --throttling-method=simulate --output=json --output-path=<scratch>` — run 3×, take the median. WSL2 caveat: requires a Chrome/Chromium binary; if none is available, use `npx playwright install chromium` and point `CHROME_PATH` at it, or fall back to the authoritative check below. Never claim a score without pasting the run's numbers.
