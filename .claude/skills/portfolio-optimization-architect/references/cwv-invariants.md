...
- **Lighthouse procedure (local proxy):** `pnpm build && pnpm start`, then against `http://localhost:3000`: `pnpm dlx lighthouse http://localhost:3000 --preset=perf --form-factor=mobile --screenEmulation.mobile --throttling-method=simulate --output=json --output-path=<scratch>` — run 3×, take the median. Chromium comes from the D-10 visual-tooling install (SKILL.md *Visual verification tooling*): `export CHROME_PATH="$(node -e "console.log(require(process.env.HOME + '/.local/share/portfolio-visual-tools/node_modules/playwright').chromium.executablePath())")"` before running Lighthouse; if Chromium won't launch, the owner runs the one-time `playwright install-deps chromium` step. Never claim a score without pasting the run's numbers.
